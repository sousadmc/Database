...
create or replace function insert_sol()
returns trigger as
$$
  begin
    if new.idCoordenador not in(
    	select idCoordenador
    	from audita natural join eventoEmergencia natural join vigia
    	where new.idCoordenador = idCoordenador and new.numCamara = numCamara
    )then raise exception 'O coordenador nao audita no local';
    end if;
    return new;
end;
$$ language plpgsql;

create trigger insert_solicita_trigger before insert on solicita for each row execute procedure insert_sol();

create or replace function insert_aloc()
returns trigger as
$$
  begin
    if new.numProcessoSocorro not in(
    	...
    	from  aciona
    	where new.numProcessoSocorro = numProcessoSocorro
            and new.numMeio = numMeio
            and new.nomeEntidade = nomeEntidade
    )then raise exception 'Meio nao acionado pelo Processo de Socorro';
    end if;
    return new;
end;
$$ language plpgsql;

create trigger insert_alocado_trigger before insert on alocado for each row execute procedure insert_aloc();
